...
class similarity:
    ...
    def pearson(self):
        ...
        # using relevant partial sums
        # YOUR CODE HERE
        deno1 = sqrt(sum_x2 - (sum_x**2) / n)
        deno2 = sqrt(sum_y2 - (sum_y**2) / n)
...
userY = sorted(userY, key=itemgetter(1), reverse = True)  
...
ratingsNN = songData[userXNN]
userXRatings = songData[userX]
for i in ratingsNN:
    if not i in userXRatings:
        userXRecos.append((i, ratingsNN[i]))
